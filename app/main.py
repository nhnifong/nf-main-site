...
# address for with the shipping rate that applies to that destination. Scoping
...
#
# `shipping_rate` is used in live mode and must be filled in with real rate IDs
# from the Stripe dashboard before going live. `shipping_rate_test` is used in
# test mode and defaults to known-good rates from the shared sample account.
SHIPPING_REGIONS = {
    "us_ca": {
        "label": "United States / Canada",
        "allowed_countries": ["US", "CA"],
        "shipping_rate": "shr_REPLACE_ME_US_CA",
        "shipping_rate_test": "shr_1Tg61JCIW3Ilocb57St9vBHl",
    },
    "intl": {
        "label": "International",
        "allowed_countries": ["JP", "KR", "GB", "DE", "FR", "AU"],
        "shipping_rate": "shr_REPLACE_ME_INTL",
        "shipping_rate_test": "shr_1Tg60eCIW3Ilocb5ICRBVyDo",
    },
}


def _shipping_rate_for(region: dict) -> str:
    """Return the shipping rate ID that matches the active Stripe mode."""
    if STRIPE_TEST_MODE:
        return region.get("shipping_rate_test") or region["shipping_rate"]
    return region["shipping_rate"]
...
async def create_checkout_session(request: Request, payload: CreateCheckoutSessionRequest):
    ...
    try:
        session = stripe_client.v1.checkout.sessions.create(params={
            ...
            "shipping_options": [
                {"shipping_rate": _shipping_rate_for(region)},
            ],
        })
    except stripe.StripeError as e:
